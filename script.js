...
const dotMenuWrap = document.getElementById("dotMenuWrap");

if (dotMenuBtn && dotMenuDropdown) {
  // Close menu when clicking outside
  document.addEventListener("click", (e) => {
    if (!dotMenuWrap.contains(e.target)) {
      dotMenuDropdown.style.display = "none";
    }
  });
  ...
  dotMenuBtn.addEventListener("click", (e) => {
    ...
    if (dotMenuDropdown.style.display === "none" || !dotMenuDropdown.style.display) {
      dotMenuDropdown.style.display = "block";
      dotMenuDropdown.classList.remove("closing");
    } else {
      dotMenuDropdown.classList.add("closing");
      setTimeout(() => {
        dotMenuDropdown.style.display = "none";
        dotMenuDropdown.classList.remove("closing");
      }, 200);
  ...
  // Menu item handlers
  const dmGames = document.getElementById("dm-games");
  const dmReport = document.getElementById("dm-report");
  const dmName = document.getElementById("dm-name");
  const dmInterests = document.getElementById("dm-interests");
  const dmMypage = document.getElementById("dm-mypage");
  ...
  if (dmGames) {
    dmGames.addEventListener("click", () => {
      closeMenu();
  ...
  if (dmReport) {
    dmReport.addEventListener("click", () => {
      closeMenu();
  ...
  if (dmInterests) {
    dmInterests.addEventListener("click", () => {
      closeMenu();
  ...
  if (dmMypage) {
    dmMypage.addEventListener("click", () => {
      closeMenu();
      console.log("My page clicked");
    });
  }

  // Close menu when partner connects
  const originalPartnerFound = socket.listeners("partnerFound")[0];
  socket.off("partnerFound");
  socket.on("partnerFound", (partner) => {
    dotMenuDropdown.style.display = "none";
    if (originalPartnerFound) originalPartnerFound.call(socket, partner);
  });

  // Ensure menu stays on top (z-index helper)
  const ensureMenuOnTop = () => {
    if (!dotMenuDropdown) return;
    try {
      const maxZ = Math.max(...Array.from(document.querySelectorAll("*")).map(el => 
        parseInt(window.getComputedStyle(el).zIndex) || 0
      ));
      if (dotMenuDropdown.style.display !== "none") {
        const menuZ = parseInt(window.getComputedStyle(dotMenuDropdown).zIndex) || 10000;
        if (menuZ <= maxZ) {
          dotMenuDropdown.style.zIndex = maxZ + 1000;
        }
      }
    } catch (e) {
  ...
  // Run z-index check periodically
  setInterval(ensureMenuOnTop, 500);
}
